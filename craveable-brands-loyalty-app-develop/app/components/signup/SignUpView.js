...
class SignUp extends Component {
	 ...
	 */
	dismiss = () => {
		const {navigation, appNav, loginSucess} = this.props;
		loginSucess();
		navigation.dispatch( 
	...
	moveBack = () => {
		...
		this.refs.SignupSwiper.scrollBy(-1);
	}

	moveForward = () => {
		if (this.state.selectedIndex === this.refs.SignupSwiper.props.children.length - 1) {
			this.props.action();
		} else {
			this.refs.SignupSwiper.scrollBy(1);
		}
					...
					<View style={{
						...
						>
							<EnterName 
								...
								updateActionTwo={this.updateLastName}
								error={error}
								updateError={this.updateError}
								copy={signUp.accountName}
							/>
							<EnterEmail 
								action={this.dismiss}
								forward={this.moveForward}
								backward={this.moveBack} 
								updateAction={this.updateEmail}
								error={error}
								updateError={this.updateError}
								copy={signUp.email}
							/>
							<EnterPhone 
								...
								forward={this.moveForward}
								backward={this.moveBack} 
								updateAction={this.updatePhone}
								error={error}
								updateError={this.updateError}
								userName={name}
								copy={signUp.mobileNumber}
							/>
							<EnterPassword 
								action={this.dismiss}
								backward={this.moveBack}
								forward={this.moveForward}
								updateAction={this.updatePassword}
