...
export default class Tranxactor {
	 ...
	 * }
	 */
	static addCard(token, data) {
		const headers = {
			...Config.tranxactor.headers,
			'Authorization': token
		};

		return axios.post(`${Config.tranxactor.api}/memberIdentifiers/mergeCards`, data, {
			headers: headers
		})
			.then(res => res)
			.catch(err => { throw err; });
	 ...
	 * }
	 */
	static resetUserPasswordToken(data) {
		try {
			const headers = {
				...Config.tranxactor.headers
			};
			
			return axios.post(`${Config.tranxactor.api}/resetToken`, data, {headers})
				.then(res => res)
				.catch(err => err);
		} catch(err) {
			console.log(err);
