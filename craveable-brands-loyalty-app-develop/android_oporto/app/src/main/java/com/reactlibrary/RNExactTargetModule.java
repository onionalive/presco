...
public class RNExactTargetModule extends ReactContextBaseJavaModule implements ETLogListener, ETPushConfigureSdkListener {
  ...
  public void initializePushManager(ReadableMap config, Promise promise) {
    System.out.println("!!! initializePushManager");
    ...
    if (!hasSdkInitialized) {
      ...
      try {
        RNExactTargetModule.etPush.configureSdk(new ETPushConfig.Builder(this.mainApplication)
                        .setEtAppId(appId)
  ...
  @ReactMethod
  public void setSubscriberKey(String key) {
    try {
      RNExactTargetModule.etPush.pushManager().setSubscriberKey(key);
    } catch (ETException e) {
      Log.e(TAG, e.getMessage(), e);
    }
  }
  ...
  public void onEvent(final PushReceivedEvent event) {
    System.out.println("!!!!!!!!!! WE GOT A PUSH NOTIFICATION");
